...
@Path("filijala")
public class EndPointFiliijala {
    ...
    @POST
    public Response kreiranjeFilijale(String parametri){
    ...
    @GET
    public Response dohvatanjeSvihFilijala() throws JMSException{
        try {
            if(context==null)context=cf.createContext();
            JMSProducer producer=context.createProducer();
            ObjectMessage om=context.createObjectMessage();
            om.setIntProperty("br", 11);
            producer.send(zahetviTema, om);
        } catch (JMSException ex) {
            Logger.getLogger(EndPointMesto.class.getName()).log(Level.SEVERE, null, ex);
        }    
            JMSConsumer consumer=context.createConsumer(odgovoriTema,"br=11");
